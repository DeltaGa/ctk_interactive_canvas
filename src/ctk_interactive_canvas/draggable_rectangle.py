...
import math
import weakref
from typing import List, Optional, Any, Tuple, Union, Iterator, TYPE_CHECKING
...
class DraggableRectangle:
    ...
    def __mul__(self, scalar: Union[int, float]) -> 'DraggableRectangle':
        ...
        if not isinstance(scalar, (int, float)):
            raise TypeError(f"unsupported operand type for *: 'DraggableRectangle' and '{type(scalar).__name__}'")
        if scalar < 0:
        ...
        return DraggableRectangle(
            self.canvas,
            center_x - new_half_width, center_y - new_half_height,
            center_x + new_half_width, center_y + new_half_height,
            dpi=self.dpi,
            outline=self.original_outline
        )
    ...
    def __and__(self, other: 'DraggableRectangle') -> Optional['DraggableRectangle']:
        ...
        """
        if not isinstance(other, DraggableRectangle):
            raise TypeError(f"unsupported operand type for &: 'DraggableRectangle' and '{type(other).__name__}'")
    ...
    def copy_(
        ...
        **kwargs
        ...
        return DraggableRectangle(
            self.canvas, new_x0, new_y0, new_x1, new_y1,
            dpi=self.dpi, **kwargs
        )
    ...
    def compare(
        cls,
        rect1: 'DraggableRectangle',
        rect2: 'DraggableRectangle'
    ) -> Tuple[bool, dict]:
        """
