...
def main():
    ...
    def create_copy():
        ...
        info_label.configure(text="Created new rectangle at offset [250, 0] using +")
    ...
    def show_union():
        ...
        canvas.create_draggable_rectangle(*bounding, outline="purple", width=1, dash=(5, 5))
    ...
    def show_coords():
        ...
        info_label.configure(
            text=f"rect1 coords via unpacking: ({x0:.0f}, {y0:.0f}, {x1:.0f}, {y1:.0f})"
        )

    ctk.CTkButton(button_frame, text="Translate Right (+=)", command=translate_right).grid(
        row=0, column=0, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Scale Up (*=)", command=scale_double).grid(
        row=0, column=1, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Scale Down (/=)", command=scale_half).grid(
        row=0, column=2, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Create Copy (+)", command=create_copy).grid(
        row=1, column=0, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Intersection (&)", command=show_intersection).grid(
        row=1, column=1, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Union (|)", command=show_union).grid(
        row=1, column=2, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Test Containment (in)", command=test_containment).grid(
        row=2, column=0, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Compare Sizes (<>)", command=compare_sizes).grid(
        row=2, column=1, padx=5, pady=5
    )
    ctk.CTkButton(button_frame, text="Show Coords (unpack)", command=show_coords).grid(
